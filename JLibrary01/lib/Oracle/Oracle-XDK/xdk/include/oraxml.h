/* $RCSfile: oraxml.h $ $Date: 11-nov-2003.15:31:00 $
...
/*---------------------------------------------------------------------------
    NAME
	XML_PRINTF [printCallback]

    FUNCTION
	Formatted output callback

    DESCRIPTION
	This macro defines a prototype for an output function to be invoked
	by the XML formatted-output code.  Instead of formatting to a stream,
	or file, or buffer, you can have the output be sent directly to a
	function callback.

    RETURNS
	(boolean) success; TRUE if OK, FALSE on error, to halt output

    ARGUMENTS
	ctx (IN) user-defined context [passed to printCallback]
...
/*---------------------------------------------------------------------------
	...
	A stream is a user defined entity here-- all that's passed in is a
	stream/context pointer, which is in turned passed to the I/O callback
	functions.  The parser does not reference the stream directly, its
...
/*---------------------------------------------------------------------------
    ...
    SEE ALSO
	printStreamEnc, printBuffer, printCallback, printSize
*/
...
/*---------------------------------------------------------------------------
    ...
    SEE ALSO
	printBufferEnc, printCallback, printStream, printSize
*/
...
/*---------------------------------------------------------------------------
    ...
    SEE ALSO
	printBuffer, printCallback, printStream, printSize
*/
...
/*---------------------------------------------------------------------------
    ...
    FUNCTION
	Prints XML tree via function callback

    DESCRIPTION
	Pretty-prints an XML document rooted at the given node via a
	user-defined callback function.  The top level is indented
	step*level spaces, the next level step*(level+1) spaces, etc.
	Output will be in the data encoding.
	...
	ctx   (IN) callback to be passed to func
	node  (IN) root node to print
	step  (IN) spaces to indent each level [0-4]
	level (IN) initial nesting level [0 for root]

    SEE ALSO
	printCallbackEnc, printBuffer, printStream, printSize, XML_PRINTF
*/
...
/*---------------------------------------------------------------------------
    NAME
	printCallbackEnc [DOM X]

    FUNCTION
	Prints XML tree via function callback in specified encoding

    DESCRIPTION
	Pretty-prints an XML document rooted at the given node via a
	user-defined callback function.  The entire document will be
	in the encoding specified (by name); if no encoding is given,
	the document is output in its current (data) encoding.
	The top level is indented step*level spaces, the next level
    ...
    RETURNS
	(size_t) total number of bytes passed to callback func

    ARGUMENTS
	func     (IN) user callback function
	ctx      (IN) user context to be passed to callback func
	node     (IN) root node to print
	step     (IN) spaces to indent each level [0-4]
	level    (IN) initial nesting level [0 for root]
	encoding (IN) printed encoding [NULL for data current encoding]
    ...
    SEE ALSO
	printCallback, printBuffer, printStream, printSize, XML_PRINTF
*/
...
/*---------------------------------------------------------------------------
    ...
    SEE ALSO
	getNodeNamespace, getNodePrefix, getNodeLocal
*/
...
/*---------------------------------------------------------------------------
    ...
    SEE ALSO
	getAttrNamespace, getAttrPrefix, getAttrLocal
*/
...
/*---------------------------------------------------------------------------
    ...
    SEE ALSO
	setCharData, createTextNode, createCommentNode, createCDATASection
*/
