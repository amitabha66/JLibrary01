/* $RCSfile: xmlproc.h $ $Date: 08-dec-2003.16:19:21 $
...
struct xmlistream; typedef struct xmlistream xmlistream;
...
/*---------------------------------------------------------------------------
    ...
    SEE ALSO
        XML_STREAM_CLOSE_F, XML_STREAM_READ_F, XML_STREAM_WRITE_F
*/
...
/*---------------------------------------------------------------------------
    ...
    SEE ALSO
        XML_STREAM_OPEN_F, XML_STREAM_READ_F, XML_STREAM_WRITE_F
*/
...
/*---------------------------------------------------------------------------
    ...
    SEE ALSO
        XML_STREAM_OPEN_F, XML_STREAM_CLOSE_F, XML_STREAM_READ_F
*/
...
/* DATATYPE xmlostream - User-defined output stream
   SEE ALSO XmlDomGetNodeValueStream, XmlDomGetAttrValueStream, XmlXslSetOutputStream, XmlXvmSetOutputStream
    Generic user-defined output stream.  The three function pointers are
    required (but may be stubs).  The context pointer is entirely user-defined;
    point it to whatever state information is required to manage the stream;
    it will be passed as first argument to the user functions.
*/
...
/*---------------------------------------------------------------------------
    ...
    DESCRIPTION
        ...
        creates and adds an attribute in a single operation.
...
/* test function, do not use - API may change */

#define XMLDOM_GET_NODE_VALUE_STREAM_CB getNodeValueStream_xmldomcb

#define XMLDOM_GET_NODE_VALUE_STREAM_F(func, xctx, node, ostream) \
    xmlerr func(xmlctx *xctx, xmlnode *node, xmlostream *ostream)

    XMLDOM_GET_NODE_VALUE_STREAM_F((*XMLDOM_GET_NODE_VALUE_STREAM_CB),
				   xctx, node, ostream);

#define XmlDomGetNodeValueStream(xctx, node, ostream) \
    (*XML_DOMCB(xctx)->XMLDOM_GET_NODE_VALUE_STREAM_CB) \
        ((xctx), (node), (ostream))
...
#define XMLDOM_SET_NODE_VALUE_STREAM_CB setNodeValueStream_xmldomcb

#define XMLDOM_SET_NODE_VALUE_STREAM_F(func, xctx, node, istream) \
    xmlerr func(xmlctx *xctx, xmlnode *node, xmlistream *istream)

    XMLDOM_SET_NODE_VALUE_STREAM_F((*XMLDOM_SET_NODE_VALUE_STREAM_CB),
				   xctx, node, istream);

#define XmlDomSetNodeValueStream(xctx, node, istream) \
    (*XML_DOMCB(xctx)->XMLDOM_SET_NODE_VALUE_STREAM_CB) \
        ((xctx), (node), (istream))
...
/****************************************************************************
    ...
    is just here to test the doc stripper.
...
/*---------------------------------------------------------------------------
    ...
    PURPOSE
        Return flag saying whether an attribute was explicitly created
...
#define XMLDOM_GET_ATTR_VALUE_STREAM_CB getAttrValueStream_xmldomcb

#define XMLDOM_GET_ATTR_VALUE_STREAM_F(func, xctx, attr, ostream) \
    xmlerr func(xmlctx *xctx, xmlnode *attr, xmlostream *ostream)

    XMLDOM_GET_ATTR_VALUE_STREAM_F((*XMLDOM_GET_ATTR_VALUE_STREAM_CB),
				   xctx, attr, ostream);

#define XmlDomGetAttrValueStream(xctx, attr, ostream) \
    (*XML_DOMCB(xctx)->XMLDOM_GET_ATTR_VALUE_STREAM_CB) \
        ((xctx), (attr), (ostream))
...
#define XMLDOM_SET_ATTR_VALUE_STREAM_CB setAttrValueStream_xmldomcb

#define XMLDOM_SET_ATTR_VALUE_STREAM_F(func, xctx, attr, istream) \
    xmlerr func(xmlctx *xctx, xmlnode *attr, xmlistream *istream)

    XMLDOM_SET_ATTR_VALUE_STREAM_F((*XMLDOM_SET_ATTR_VALUE_STREAM_CB),
				   xctx, attr, istream);

#define XmlDomSetAttrValueStream(xctx, attr, istream) \
    (*XML_DOMCB(xctx)->XMLDOM_SET_ATTR_VALUE_STREAM_CB) \
        ((xctx), (attr), (istream))
