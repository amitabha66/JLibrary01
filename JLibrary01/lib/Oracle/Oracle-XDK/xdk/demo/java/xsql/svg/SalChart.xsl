<?xml version="1.0"?>
<!--
 | smuench 08/24/02 - Update media type to image/svg+xml
 +-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
 ...
 <xsl:template name="drawBar">
   ...
   <xsl:variable name="scale"     select="number(0.40)"/>
   <xsl:variable name="xOffset"   select="number(120)"/>
   <xsl:variable name="yOffset"   select="number(20)"/>
   <xsl:variable name="barHeight" select="number(20)"/>
   ...
   <rect x="{$x}" y="{$y}" height="{$barHeight}" width="{$barWidth * $scale}">
   ...
   <xsl:variable name="x2" select="$xOffset + $barWidth * $scale + 10"/>
